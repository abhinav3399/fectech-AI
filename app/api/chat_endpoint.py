...
async def chat_query(text: str = Body(..., embed=True)):
    ...
    # Graceful fallback: no memory match found, but still answer warmly via the
    # LLM instead of dead-ending. Keeps the assistant always responsive.
    fallback_text = llm_service.generate_response(user_text=text, context=None)
    return {
        "status": "general",
        "text": fallback_text
    }
